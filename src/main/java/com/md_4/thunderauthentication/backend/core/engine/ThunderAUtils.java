...
import com.md_4.thunderauthentication.backend.core.engine.commands.ThunderAuth;
import com.md_4.thunderauthentication.backend.core.engine.tools.NameFetcher;
import com.md_4.thunderauthentication.backend.core.engine.tools.UUIDFetcher;
...
public class ThunderAUtils {
	...
	private static BukkitTask createTitleReminder(final Player p, Plugin pl, int fedin, int stay, int fedout) {
		final Runnable r = new Runnable() {
			@Override
			public void run() {
				String Title = Format.color("&a&l" + Language.LOGIN_TITLE.toString());
				String SubTitle = Format.color("&e" + Language.LOGIN_SUBTITLE.toString());
				p.sendTitle(Title, SubTitle, fedin, stay, fedout);
			}
		};
		return pl.getServer().getScheduler().runTaskTimer(pl, r, 0L, 200);
	}
    ...
    private static BukkitTask createReminder(final Player p, Plugin pl, final String message, int fedin, int stay, int fedout, final long interval) {
        final Runnable r = new Runnable() {
            @Override
            public void run() {
		...
		new BukkitRunnable() {
		    ...
		    public void run() {
	            final Runnable r = new Runnable() {
      	        ...
      	        Bukkit.getServer().getScheduler().runTaskLater(Main.getInstance(), r, NumberOfTicks);
		    }
    ...
    public static BukkitTask createRegisterReminder(Player p, Plugin pl) {
		...
		return createReminder(p, pl, SendMsg + " [" + RegType + "]" + ChatColor.RED + "!" + ExtMsg, 3, 10, 3, Config.remindInterval * 20L);
    }
    ...
    public static BukkitTask createSetEmailReminder(Player p, Plugin pl) {
	    ...
	    return createReminder(p, pl, ChatColor.RED + colorize(Language.PLEASE_SETEMAIL_WITH.toString()) + ":" + ChatColor.GRAY + " /setemail [" + RegType + "]" + ChatColor.RED + "!", 3, 10, 3,Config.emailRemindInterval * 20L);
    }
	...
	public static BukkitTask createTitleLoginReminder(Player p, Plugin pl) {
		String RegType = colorize(Language.PASSWORD.toString()); //"password";
		String SendMsg;
		String ExtMsg="";
		String Command="";
		...
		if(Config.sessionType.contains("HiddenChat"))
			...
			SendMsg = ChatColor.RED + colorize(Language.PLEASE_LOG_IN_WITH1.toString()) + ":" + ChatColor.GRAY + " " + Command;
		return createTitleReminder(p, pl, 3, 30 ,3 );
	}
    ...
    public static BukkitTask createRunLaterCommand(String PlayerName, String CommandUsingTWait) {
		//Get value first parameter (not explicit "TWait()" and remove the parameter from the string
		CommandUsingTWait = CommandUsingTWait.trim();
    	String CommandToSend= CommandUsingTWait.substring(CommandUsingTWait.indexOf(' ')+1);

		long NumberOfTicks=0L;
		try {
			NumberOfTicks = Long.parseLong(CommandUsingTWait.split("[\\(\\)]")[1]); // In Ticks
	        return createRunLater(PlayerName, CommandToSend, NumberOfTicks);
		} catch (NumberFormatException nfe) {
			//System.out.println("NumberFormatException: " + nfe.getMessage());
			Main.getInstance().getLogger().info("Error TWait() NumberFormatException: (" + NumberOfTicks + ") " + CommandUsingTWait);
			nfe.printStackTrace();
		}
		return null;
    }
    ...
    /**
     * Creates a task to Check ThunderAuthentication current version.
     * TaskTimer interval is in-game ticks, so 1 second = 20 ticks
     * To wait 1 minute set: minutes * 60 * 20 ticks interval on TaskTimer. 
     *
     * @param pl       Plugin to register the task under
    ...
    public static BukkitTask ChkVersionTimerExec(Plugin pl) {
    	...
    	//BukkitTask thisTask = pl.getServer().getScheduler().runTaskTimer(pl, r, 0L, interval);
    	BukkitTask thisTask = pl.getServer().getScheduler().runTaskTimer(pl, r, interval, interval);
    	pl.getLogger().info("Check-ThunderAuthentication-Version Task is started (interval: " + daysInterval +" days)" );
        return thisTask;
        //return pl.getServer().getScheduler().runTaskTimer(pl, r, 0L, interval);
    }
    ...
    public static UUID getUUID(String name) throws Exception {
		...
		{
    		// Server runs 'OffLine' ThunderAuthentication calculates the UUID for this player...
