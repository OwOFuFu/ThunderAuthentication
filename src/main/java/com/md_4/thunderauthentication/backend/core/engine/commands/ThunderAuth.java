...
public class ThunderAuth implements CommandExecutor {
    ...
    public ThunderAuth(Main instance) {
        this.plugin = instance;
    }

    @Override
    public boolean onCommand(final @NotNull CommandSender cs, Command cmd, @NotNull String label, String[] args) {
        if ( !cmd.getName().equalsIgnoreCase("")) {
            if (!cs.hasPermission("thunderauthentication.")) {
            ...
            switch (subcommand) {
                case "help": {
                    cs.sendMessage(ChatColor.BLUE + ThunderAUtils.colorize(Language.ADMIN_HELP.toString()));
                    cs.sendMessage(ChatColor.GRAY + "  /" + label + " changepassword [player] [newpassword]" + ChatColor.BLUE + " - " + ThunderAUtils.colorize(Language.HELP_CHANGEPASSWORD.toString()));
                    cs.sendMessage(ChatColor.GRAY + "  /" + label + " login [player]" + ChatColor.BLUE + " - " + ThunderAUtils.colorize(Language.HELP_LOGIN.toString()));
                    cs.sendMessage(ChatColor.GRAY + "  /" + label + " logout [player]" + ChatColor.BLUE + " - " + ThunderAUtils.colorize(Language.HELP_LOGOUT.toString()));
                    cs.sendMessage(ChatColor.GRAY + "  /" + label + " register [player] [password]" + ChatColor.BLUE + " - " + ThunderAUtils.colorize(Language.HELP_REGISTER.toString()));
                    cs.sendMessage(ChatColor.GRAY + "  /" + label + " setemail [player] [email-address]" + ChatColor.BLUE + " - " + ThunderAUtils.colorize(Language.HELP_REGEMAIL.toString()));
            		cs.sendMessage(ChatColor.GRAY + "  /" + label + " testemail [player]" + ChatColor.BLUE + " - Test Email-Setup, send test-mail to [Player]");
                    cs.sendMessage(ChatColor.GRAY + "  /" + label + " unregister [player]" + ChatColor.BLUE + " - " + ThunderAUtils.colorize(Language.HELP_UNREGISTER.toString()));
                    cs.sendMessage(ChatColor.GRAY + "  /" + label + " getuuid [player]" + ChatColor.BLUE + " - " + ThunderAUtils.colorize(Language.HELP_GETUUID.toString()));
                    cs.sendMessage(ChatColor.GRAY + "  /" + label + " nlplist " + ChatColor.BLUE + " - " + ThunderAUtils.colorize(Language.HELP_NLPLIST.toString()));
                    cs.sendMessage(ChatColor.GRAY + "  /" + label + " nlpadd [player]" + ChatColor.BLUE + " - " + ThunderAUtils.colorize(Language.HELP_NLPADD.toString()));
                    cs.sendMessage(ChatColor.GRAY + "  /" + label + " nlprem [player]" + ChatColor.BLUE + " - " + ThunderAUtils.colorize(Language.HELP_NLPREM.toString()));
            		...
            		cs.sendMessage(ChatColor.GRAY + "  /" + label + " showstats" + ChatColor.BLUE + " - Show ThunderAuthentication Player-base Statistics ");
            		cs.sendMessage(ChatColor.GRAY + "  /" + label + " chkdevmsg" + ChatColor.BLUE + " - Show/check info/version from Plugin ");
                    cs.sendMessage(ChatColor.GRAY + "  /" + label + " reload" + ChatColor.BLUE + " - " + ThunderAUtils.colorize(Language.HELP_RELOAD.toString()));
                    cs.sendMessage(ChatColor.GRAY + "  /" + label + " debug" + ChatColor.BLUE + " - Debug commands, use with caution!!!");   
                    cs.sendMessage(ChatColor.GRAY + "  /" + label + " help" + ChatColor.BLUE + " - " + ThunderAUtils.colorize(Language.HELP_HELP.toString()));
                    break;
            	}
                case "changepassword": {
                    //if (cs instanceof Player) ((Player) cs).getLocation().getWorld().setGameRuleValue("logAdminCommands", "true");
                    if (args.length < 3) {
                        cs.sendMessage(ChatColor.RED + ThunderAUtils.colorize(Language.NOT_ENOUGH_ARGUMENTS.toString() + " " + Language.TRY.toString()) + " " + ChatColor.GRAY + "/" + label + " help" + ChatColor.RED + ".");
                        return true;
                    }
                    AuthPlayer ap = AuthPlayer.getAuthPlayer(args[1]);
                    
                    if (ap == null) {
                        cs.sendMessage(ChatColor.RED + ThunderAUtils.colorize(Language.ERROR_OCCURRED.toString()));
                        return true;
                    }
                    if (!ap.isRegistered()) {
                        cs.sendMessage(ChatColor.RED + ThunderAUtils.colorize(Language.PLAYER_NOT_REGISTERED.toString()));
                        return true;
                    }
                    if (ap.setPassword(args[2], Config.passwordHashType)) {
                        cs.sendMessage(ChatColor.BLUE + ThunderAUtils.colorize(Language.PASSWORD_CHANGED.toString()));
                        ap.setLastJoinTimestamp(System.currentTimeMillis()); // Force ProfileSave
                    }
                    else cs.sendMessage(ChatColor.RED + ThunderAUtils.colorize(Language.PASSWORD_COULD_NOT_BE_CHANGED.toString()));
                ...
                case "login": {
                    ...
                    if (p == null) {
                        cs.sendMessage(ChatColor.RED + ThunderAUtils.colorize(Language.PLAYER_NOT_ONLINE.toString()));
                        return true;
                    }
                    ap.login();
                    this.plugin.getLogger().info(p.getName() + " " + Language.HAS_LOGGED_IN);
                    cs.sendMessage(ChatColor.BLUE + ThunderAUtils.colorize(Language.PLAYER_LOGGED_IN.toString()));
                    break;
                }
                case "logout": {
                    if (args.length < 2) {
                    ...
                    if (!ap.isLoggedIn()) {
                        cs.sendMessage(ChatColor.RED + ThunderAUtils.colorize(Language.PLAYER_NOT_LOGGED_IN.toString()));
                        return true;
                    }
                    ap.logout(this.plugin, true);
                    cs.sendMessage(ChatColor.BLUE + ThunderAUtils.colorize(Language.PLAYER_LOGGED_OUT.toString()));
                    break;
                }
                case "register": {
                    //if (cs instanceof Player) ((Player) cs).getLocation().getWorld().setGameRuleValue("logAdminCommands", "true");
                    ...
                    if (ap.isRegistered()) {
                        cs.sendMessage(ChatColor.RED + ThunderAUtils.colorize(Language.PLAYER_ALREADY_REGISTERED.toString()));
                        return true;
                    }
                    ...
                    if (ap.setPassword(rawPassword, Config.passwordHashType)) {
                    	...
                    	cs.sendMessage(ChatColor.BLUE + String.format(ThunderAUtils.colorize(Language.REGISTERED_SUCCESSFULLY.toString()), ChatColor.GRAY + args[1] + ChatColor.BLUE));
                    	...
                    	cs.sendMessage(ChatColor.RED + String.format(ThunderAUtils.colorize(Language.COULD_NOT_REGISTER.toString()), ChatColor.GRAY + args[1] + ChatColor.RED));
                    break;
                }
                case "setemail": {
                    //if (cs instanceof Player) ((Player) cs).getLocation().getWorld().setGameRuleValue("logAdminCommands", "true");
                    ...
                    if (!EmailAddress.contains("@")) {
                        cs.sendMessage(ChatColor.RED + String.format(ThunderAUtils.colorize(Language.PLAYER_INVALID_EMAILADDRESS.toString() + "."), args[1], EmailAddress));
                        return true;
                    }
                    ...
                    if (ap.setEmailAddress(EmailAddress)) {
                    	ap.setLastJoinTimestamp(System.currentTimeMillis()); // Force ProfileSave                    	
                        //cs.sendMessage(ChatColor.BLUE + String.format(Language.REGISTERED_SUCCESSFULLY.toString(), ChatColor.GRAY + name + ChatColor.BLUE));
                        String name = ThunderAUtils.forceGetName(ap.getUniqueId());
                        if(name!=args[1]) ap.setUserName(args[1]); //name not set?, set it!
                    	cs.sendMessage(ChatColor.BLUE + String.format(ThunderAUtils.colorize(Language.REGISTERED_SUCCESSFULLY.toString()), ChatColor.GRAY + args[2] + ChatColor.BLUE));
                ...
                case "unregister": {
                	...
                	cs.sendMessage(ChatColor.BLUE + ThunderAUtils.colorize(Language.PLAYER_REMOVED.toString()));
                    break;
                }
                case "getuuid": {
                    if (args.length < 2) {
                    ...
                    UUID u;
                	try {
                		u = ThunderAUtils.getUUID(args[1]);
                	} catch (Exception ex) {
                		//ex.printStackTrace();
                        u = null;        		
                    }
                ...
                case "nlpadd": {
                    ...
                    ap.setVIP(true);
                    cs.sendMessage(ChatColor.BLUE + String.format(ThunderAUtils.colorize(Language.NLP_SET_UPDATED.toString()), ChatColor.GRAY + name + ChatColor.BLUE));
                    break;
                }
                case "nlprem": {
                    if (args.length < 2) {
                ...
                case "testqueue": {
                	Main.MyQueue.Put("QueueCallBack:~This is the test-data");
                    break;
                }                
                case "chkdevmsg": {
                	CheckDevMessage(cs);
                ...
                case "reload": {
                    Bukkit.getPluginManager().getPlugin("ThunderAuthentication").reloadConfig();
                    this.plugin.c.reloadConfiguration(); // Do both ??
                    cs.sendMessage(ChatColor.BLUE + ThunderAUtils.colorize(Language.CONFIGURATION_RELOADED.toString()));
                ...
                case "debug": {
                    if (args.length < 2) {
                		...
                		cs.sendMessage(ChatColor.GRAY + "  /" + label + " listusers" + ChatColor.BLUE + " - (debug) Show Player list (ActiveProfile)");
                        cs.sendMessage(ChatColor.GRAY + "  /" + label + " listqueue" + ChatColor.BLUE + " - (debug) show current TQueue entries");
                        cs.sendMessage(ChatColor.GRAY + "  /" + label + " testqueue" + ChatColor.BLUE + " - (debug) test working of TQueue system");
                    	
                        cs.sendMessage(ChatColor.RED + ThunderAUtils.colorize(Language.NOT_ENOUGH_ARGUMENTS.toString() + " " + Language.TRY.toString()) + " " + ChatColor.GRAY + "/" + label + " debug <name> [on|off]" + ChatColor.RED + ".");
                    ...
                    if (args[1].equals("queuemgr")) cs.sendMessage("debug queuemgr set to: " + Main.MyQueue.SetDebugQueue(CmpParm));

                    break;
                }

                default: {
                    cs.sendMessage(ChatColor.RED + ThunderAUtils.colorize(Language.INVALID_SUBCOMMAND.toString() + " " + Language.TRY.toString()) + " " + ChatColor.GRAY + "/" + label + " help" + ChatColor.RED + ".");
                    break;
                }
            }
            return true;
    ...
    public static void CheckDevMessage(final CommandSender cs) {
        new Thread(() -> {
            try {
                ...
                URL url = new URL("https://github.com/md4Studios");

                String ThisPlugin= Main.getInstance().getDescription().getName();
                String ThisVersion= Main.getInstance().getDescription().getVersion();
...
String[] dMsg = Msg[1].split("-");
                        ...
                        if(dMsg.length<2)
                           ...
                           if(strPlgVersion.compareTo(strWebVersion)==0) {
                               System.out.println("[" + ThisPlugin + "] You have the Latest ThunderAuthentication Version (" + ThisVersion + ") running.");
                           } else {
                               if(strPlgVersion.compareTo(strWebVersion) < 0) {
                                      if(strWebVersion.contains("BETA"))
                                          System.out.println("[" + ThisPlugin + "] There is a new BETA Version (" + Msg[1] + ") of the ThunderAuthentication plugin available!");
                                      else
                                          System.out.println("[" + ThisPlugin + "] There is a newer Version (" + Msg[1] + ") of the ThunderAuthentication plugin available!");
