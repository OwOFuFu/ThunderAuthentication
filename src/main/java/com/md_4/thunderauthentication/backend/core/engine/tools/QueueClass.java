...
// ThunderAuthentication QueueClass to Implement String Queue
...
{
    protected ConcurrentLinkedDeque<String> TQueue = new ConcurrentLinkedDeque<String>();
    ...
    public void DisplayQueue(final CommandSender cs) {
		new BukkitRunnable() {
		    @Override
		    public void run() {
		    	cs.sendMessage("Entries in TQueue: ");
		        Iterator<String> itr = TQueue.iterator();
				...
				public void run() {
					...
					while(!isEmpty()){
			    		...
			    		if(Msg[0].equals("QueueCallBack:")) { // Run QueueCallBack You Name it....
			    			ThunderAuth.QueueCallBack(Msg[1]); // Just for Debug/testing
			    		}
			    		// ------------------------------------------------------------------------------
