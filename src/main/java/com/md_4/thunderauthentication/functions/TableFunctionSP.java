...
public class TableFunctionSP {
    ...
    public static void createTable(Player player, UUID uuid) throws Exception {
        ...
        stmt.executeUpdate("INSERT INTO `"+ player.getName().toString() +"`(`name`, `uuid`, `offlineuuid` , `premium`) " + "VALUES('" + player.getName().toString() + "','" + MojangAPI.getUUID(player.getName()).toString() + "', '" + UUIDFetcher.getOFFUUID(player.getName()).toString() + "','NO')");
    }
}
